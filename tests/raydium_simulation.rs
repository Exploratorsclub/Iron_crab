...
async fn raydium_swap_plan_simulation_layout() {
    ...
    if let Err(e) = raydium.refresh_pools().await {
        eprintln!("skip: refresh failed {e}");
        return;
    }
    let snaps = raydium.snapshots();
    if snaps.is_empty() {
        eprintln!("skip: no pools fetched");
        return;
    }
    ...
    let plan_opt = raydium
        ...
        .expect("plan build result");
    if plan_opt.is_none() {
        eprintln!("skip: no plan for chosen pool");
        return;
    }
