...
fn test_config_update_roundtrip() {
    ...
    config.insert("max_slippage_bps".to_string(), serde_json::json!(500));
    config.insert(
    ...
    let response = ConfigUpdateResponse {
        ...
        rejected_keys: vec![
            (
                ...
                "Unknown config key: unknown_key".to_string(),
            ),
...
fn test_config_update_response_rejected() {
    let response = ConfigUpdateResponse {
        ...
        applied_keys: vec![],
        rejected_keys: vec![("bad_key".to_string(), "Unknown config key".to_string())],
        new_snapshot_id: None,
    };
...
fn test_config_value_types() {
    ...
    // u64 values
    config.insert(
        "max_position_size_lamports".to_string(),
        serde_json::json!(500_000_000u64),
    );
    config.insert(
        "daily_loss_limit_lamports".to_string(),
        serde_json::json!(5_000_000_000u64),
    ...
    config.insert(
        "default_position_lamports".to_string(),
        serde_json::json!(100_000_000u64),
    );

    // u32 values (BPS)
    config.insert("max_slippage_bps".to_string(), serde_json::json!(500u32));
    config.insert(
        "early_max_slippage_bps".to_string(),
        serde_json::json!(300u32),
    );

    // f64 values
    config.insert(
        "early_min_liquidity_sol".to_string(),
        serde_json::json!(5.0f64),
    );
    ...
    let update = ConfigUpdate::new("test", "test-build", "test-run", "test", config);

    // Verify type extraction works correctly
    assert_eq!(
        update
            .config
            .get("max_position_size_lamports")
            .and_then(|v| v.as_u64()),
        Some(500_000_000)
    );
    assert_eq!(
