...
async fn partial_exit_state_reduction() {
    ...
    let res = engine.test_apply_partial_reduction(&mint, 0, fraction, realized_delta).expect("lot exists");
    ...
    assert!((remaining_invested - invested_sol * (1.0 - fraction)).abs() < 1e-9, "invested capital not reduced proportionally");
    assert!((remaining_tokens - amount_tokens * (1.0 - fraction)).abs() < 1e-3, "token amount not reduced proportionally");
    assert!((realized_added - realized_delta).abs() < 1e-12, "realized delta not applied");

    // Full exit of remaining lot
    let res2 = engine.test_apply_partial_reduction(&mint, 0, 1.0, remaining_invested).expect("second reduction");
