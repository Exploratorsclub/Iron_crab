use ironcrab::backtest::{engine::{BacktestEngine,NoopStrategy, make_swap}, market::{CfmAdapter,CfmPool}, types::{Portfolio,SimEvent,SimEventKind,StrategyDecision,BacktestStrategy,SimContext}};

struct OneSwap;
impl BacktestStrategy for OneSwap {
    fn on_event(&self, _ctx:&SimContext, _ev:&SimEvent) -> StrategyDecision {
        StrategyDecision { actions: vec![make_swap("P1","A","B", 50_000, 10)] } // 0.10% max slippage
    }
}

fn mk_events() -> Vec<SimEvent> {
    vec![
        SimEvent { ts_ms: 1, kind: SimEventKind::SlotAdvance { slot: 1 } },
        SimEvent { ts_ms: 2, kind: SimEventKind::Log("noop".into()) },
    ]
}
