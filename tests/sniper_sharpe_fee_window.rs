...
use ironcrab::solana::sniper::{SniperEngine, SniperCfg};
...
async fn sharpe_lower_with_fees() {
    ...
    let base_cfg = SniperCfg { rolling_pnl_window: Some(20), ..SniperCfg::default() };
    let eng_no_fee = SniperEngine::new(rpc.clone(), base_cfg.clone(), None, None, temp_treasury("fee0"));
    ...
    for (f, r) in fractions.into_iter().zip(gross_returns.into_iter()) {
        // No-fee engine
        let slice_no_fee = eng_no_fee.test_current_invested_for_lot(&mint_a, 0).unwrap() * f;
        let proceeds_no_fee = slice_no_fee * (1.0 + r);
        let _ = eng_no_fee.test_simulate_partial_exit_with_fee(&mint_a, 0, f, proceeds_no_fee, 0.0).unwrap();
        ...
        let _ = eng_fee.test_simulate_partial_exit_with_fee(&mint_b, 0, f, proceeds_fee, fee_sol).unwrap();
    }
    ...
    assert!(sharpe_fee < sharpe_no_fee, "Sharpe with fees should be lower ({} !< {})", sharpe_fee, sharpe_no_fee);
}

#[tokio::test]
async fn rolling_window_truncation() {
    let rpc = Arc::new(SolanaRpc::new("http://localhost:8899"));
    let cfg = SniperCfg { rolling_pnl_window: Some(5), ..SniperCfg::default() };
    ...
    for _ in 0..8 {
        ...
        let _ = engine.test_simulate_partial_exit_with_fee(&mint, 0, 0.1, proceeds, 0.0).unwrap();
        let (_sharpe, count) = engine.test_get_sharpe();
        assert!(count <= 5, "recent_realized should not exceed window, got {count}");
    }
