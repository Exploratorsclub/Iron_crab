...
#[derive(Clone)]
struct EdgeDex { a: String, b: String, multiplier: u64 } // multiplier in bps (10000 = 1x)
...
async fn profit_ranking_orders_cycles() {
    ...
    let connectors: Vec<Arc<dyn Dex>> = vec![
        Arc::new(EdgeDex { a: "A".into(), b: "B".into(), multiplier: 10_200 }),
        Arc::new(EdgeDex { a: "B".into(), b: "C".into(), multiplier: 10_100 }),
        Arc::new(EdgeDex { a: "C".into(), b: "A".into(), multiplier: 10_200 }),
        Arc::new(EdgeDex { a: "A".into(), b: "X".into(), multiplier: 10_300 }),
        Arc::new(EdgeDex { a: "X".into(), b: "Y".into(), multiplier: 10_200 }),
        Arc::new(EdgeDex { a: "Y".into(), b: "A".into(), multiplier: 10_200 }),
    ];
    let engine = ArbitrageEngine::new(rpc, connectors).with_profit_params(0, 0);
    let ranked = engine.rank_triangular_cycles(&["A".into()], 1_000_000u64, 5).await.unwrap();
