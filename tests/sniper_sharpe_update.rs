...
async fn sharpe_updates_after_partial_exits() {
    ...
    let cfg = SniperCfg { rolling_pnl_window: Some(20), ..SniperCfg::default() };
    // temp keypair file
    let tmp_path = {
    ...
    let current_invest_slice = engine.test_current_invested_for_lot(&mint, 0).unwrap() * f;
        ...
        let fee = 0.0;
        let _ = engine.test_simulate_partial_exit_with_fee(&mint, 0, f, proceeds, fee).expect("simulation");
    }

    let (sharpe, count) = engine.test_get_sharpe();
    assert!(count >= 5, "need at least 5 samples for Sharpe, got {count}");
    assert!(sharpe.is_finite() && sharpe.abs() > 0.0, "Sharpe should be computed and non-zero, got {sharpe}");
}
