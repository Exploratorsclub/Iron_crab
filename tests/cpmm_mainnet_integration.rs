...
async fn test_cpmm_wsol_useless_pool() {
    let rpc_url = std::env::var("SOLANA_RPC_URL")
        .unwrap_or_else(|_| "https://mainnet.helius-rpc.com/?api-key=96755862-7b83-484a-9f7a-2c0620253cc1".to_string());
    ...
    match cpmm.quote_exact_in(wsol, useless, amount_in).await {
        Ok(Some(quote)) => {
            ...
            assert!(quote.price_impact_bps < 10000, "Price impact should be reasonable");
    ...
    match cpmm.quote_exact_in(useless, wsol, amount_in_useless).await {
        Ok(Some(quote)) => {
            ...
            println!("   Amount Out: {} lamports (~{} SOL)", quote.amount_out, quote.amount_out as f64 / 1e9);
        }
        Ok(None) => {
