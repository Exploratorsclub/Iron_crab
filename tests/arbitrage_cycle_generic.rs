...
#[derive(Clone)]
struct EdgeDex { a: String, b: String, mul_bps: u64 }
...
async fn enumerate_4hop_cycle() {
    ...
    let connectors: Vec<Arc<dyn Dex>> = vec![
        ...
        Arc::new(EdgeDex { a: "C".into(), b: "D".into(), mul_bps: 10_050 }),
        Arc::new(EdgeDex { a: "D".into(), b: "A".into(), mul_bps: 10_020 }),
    ];
    let engine = ArbitrageEngine::new(rpc, connectors).with_profit_params(0, 0);
    let cycles = engine.enumerate_cycles_generic(&["A".into()], 1_000_000u64, 5, 20).await.unwrap();
    assert!(!cycles.is_empty());
    let has_abcd = cycles.iter().any(|c| c.path.len()==5 && c.path[0]=="A" && c.path[1]=="B" && c.path[2]=="C" && c.path[3]=="D" && c.path[4]=="A");
