...
const CONFIG_DESCRIPTIONS: Record<string, Record<string, string>> = {
  ...
  'momentum-bot': {
    ...
    max_hold_time_secs: 'EXIT: Max hold time before TIME_EXIT eligible (seconds)',
    max_hold_absolute_cap_secs: 'EXIT: Absolute max hold; TIME_EXIT regardless of velocity (0=off)',
    time_exit_requires_low_velocity: 'EXIT: TIME_EXIT only when trades_per_min < min_trades_per_min',
    momentum_exit_min_hold_secs: 'EXIT: Min hold before MOMENTUM_EXIT (seconds)',
    momentum_exit_only_when_losing: 'EXIT: Momentum exit only when PnL <= max PnL threshold',
    momentum_exit_max_pnl_pct: 'EXIT: Max PnL % for momentum exit when only_when_losing is false',
