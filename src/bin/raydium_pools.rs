...
fn main() -> anyhow::Result<()> {
    ...
    match mode {
        Some(Mode::Mint(m)) => {
            ...
            for p in pools {
                println!(
                    "{} | baseMint={} quoteMint={} lpMint={} market={}",
        ...
        Some(Mode::Pair(a, b)) => {
            let mut v =
                reader::fetch_pools(&rpc, Some(a), Some(b), active_only, false, program_id)?;
            if either {
                let mut r =
                    reader::fetch_pools(&rpc, Some(b), Some(a), active_only, false, program_id)?;
                v.append(&mut r);
                v.sort_by_key(|p| p.address);
                v.dedup_by_key(|p| p.address);
            }
            println!(
                "Found {} pools for pair ({}, {}){}",
                v.len(),
                a,
                b,
