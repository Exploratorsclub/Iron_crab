...
#[derive(Parser, Debug)]
#[command(name="ironcrab-recorder", about="Record slots, logs, and accounts to a compressed JSONL trace for deterministic replays")]
struct Opts {
...
async fn main() -> anyhow::Result<()> {
    ...
    let rpc_url = opts.rpc_url.clone().or_else(|| std::env::var("SOLANA_RPC").ok()).unwrap_or_else(|| "https://api.mainnet-beta.solana.com".to_string());
    let rpc = Arc::new(SolanaRpc::new(&rpc_url));
    let ws_url = if rpc_url.starts_with("https://") { rpc_url.replacen("https://", "wss://", 1) } else { rpc_url.replacen("http://", "ws://", 1) };
    ...
    for pid_str in [RAYDIUM_AMM_V4, ORCA_WHIRLPOOL_PROGRAM] {
        ...
        let handle = tokio::spawn(async move {
            ...
            if let Ok((mut ws,_)) = connect_async(&ws_url).await {
                ...
                while let Some(msg) = ws.next().await { if let Ok(Message::Text(txt)) = msg {
                    if txt.contains("logsNotification") {
                        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&txt) {
                            if let Some(arr) = v.pointer("/params/result/value/logs").and_then(|x| x.as_array()) {
                                if let Some(slot) = v.pointer("/params/result/context/slot").and_then(|x| x.as_u64()) {
                                    // write Slot event (idempotent in downstream)
                                    let slot_ev = TraceEvent::Slot { slot };
                                    let _ = tx_logs.send(serde_json::to_string(&slot_ev).unwrap());
                                    for e in arr { if let Some(line) = e.as_str() {
                                        let log_ev = TraceEvent::Log { slot, msg: line.to_string() };
                                        let _ = tx_logs.send(serde_json::to_string(&log_ev).unwrap());
                                    } }
                                }
                            }
                        }
    ...
    loop {
        ...
        if opts.max_secs > 0 && start.elapsed() >= Duration::from_secs(opts.max_secs) { break; }
        let mut tasks = Vec::new();
        if opts.raydium {
            let rpc_c = rpc.clone();
            let tx_c = tx.clone();
            tasks.push(tokio::spawn(async move {
                if let Err(e) = dump_program_accounts(&rpc_c, RAYDIUM_AMM_V4, tx_c).await { eprintln!("raydium dump error: {e}"); }
            }));
        }
        if opts.orca {
            let rpc_c = rpc.clone();
            let tx_c = tx.clone();
            tasks.push(tokio::spawn(async move {
                if let Err(e) = dump_program_accounts(&rpc_c, ORCA_WHIRLPOOL_PROGRAM, tx_c).await { eprintln!("orca dump error: {e}"); }
            }));
        }
    ...
    for h in subs { let _ = h.abort(); }
    drop(tx);
    Ok(())
}

async fn dump_program_accounts(rpc: &Arc<SolanaRpc>, program_id_str: &str, tx: tokio::sync::mpsc::UnboundedSender<String>) -> anyhow::Result<()> {
    let program_id = Pubkey::from_str(program_id_str)?;
    let cfg = RpcProgramAccountsConfig {
        filters: None,
        account_config: RpcAccountInfoConfig { encoding: None, data_slice: None, commitment: None, min_context_slot: None },
        with_context: None,
        sort_results: None,
    };
    let pairs = rpc.rpc.get_program_accounts_with_config(&program_id, cfg).await?;
    for (addr, acc) in pairs {
        let data_b64 = base64::engine::general_purpose::STANDARD.encode(&acc.data);
        let ev = TraceEvent::Account { pubkey: addr.to_string(), data_b64 };
