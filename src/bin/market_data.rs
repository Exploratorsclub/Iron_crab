...
use ironcrab::market_data::cold::{
    ...
    cold_path_rpc_refresh_meteora_dlmm_pool_row, cold_path_rpc_refresh_orca_whirlpool_pool_row,
    handle_ensure_meteora_cpmm_pool_state, handle_ensure_meteora_dlmm_pool_state,
    handle_ensure_orca_whirlpool_pool_state, handle_ensure_pump_amm_pool_accounts,
    handle_ensure_pumpfun_bonding_curve, handle_ensure_raydium_amm_pool_state,
    handle_ensure_raydium_cpmm_pool_state, ColdHost,
};
...
impl SidefxWorkerHost for MarketDataSidefxHost {
    ...
    fn maybe_spawn_raydium_serum_cold_backfill(&self, pool: Pubkey, state: &RaydiumAmmState) {
        use ironcrab::execution::live_pool_cache::raydium_amm_serum_needs_cold_backfill;
...
struct MarketDataContext {
    ...
    /// Local RPC for FIX-29 Raydium serum backfill spawns (cold path only; set at Geyser-loop start).
...
async fn run_geyser_loop(
    ...
    *ctx.cold_path_rpc.write() = Some(Arc::clone(&rpc));
...
mod pr_b_geyser_tracking_tests {
    ...
    #[test]
    fn cache_publish_not_skipped_for_momentum_hot_with_live_vault_feed() {
        let tmp = tempfile::TempDir::new().expect("tempdir");
        let jsonl_cfg = JsonlWriterConfig::new("market_events").with_log_dir(tmp.path());
        let jsonl = QueuedJsonlWriter::spawn(jsonl_cfg, 256).expect("jsonl");
        let ctx = minimal_market_data_context_for_pr_d_tests(jsonl);
        ...
        *ctx.last_synced_explicit_pubkeys.write() =
        ...
        assert!(ctx.pool_has_live_vault_geyser_feed(pool));
        assert!(
            !ctx.balance_updated_from_cache_skipped_for_live_feed(pool),
            "momentum-hot pool must bypass live-feed skip for JetStream SLAVE refresh"
        );
    }

    #[test]
    fn cache_publish_not_skipped_for_momentum_hot_pumpfun_with_live_curve_feed() {
        let tmp = tempfile::TempDir::new().expect("tempdir");
        let jsonl_cfg = JsonlWriterConfig::new("market_events").with_log_dir(tmp.path());
        let jsonl = QueuedJsonlWriter::spawn(jsonl_cfg, 256).expect("jsonl");
        let ctx = minimal_market_data_context_for_pr_d_tests(jsonl);
        ...
        assert!(
            !ctx.balance_updated_from_cache_skipped_for_live_feed(pool),
            "momentum-hot PumpFun with synced bonding curve must not skip cache publish"
        );
        assert!(
            cached_pool_has_fresh_reserve_basis(&ctx.live_pool_cache.get(&pool).unwrap()),
            "PumpFun fresh basis must be publishable"
        );
    }

    #[test]
    fn momentum_pin_register_success_triggers_cache_publish_path() {
        use ironcrab::metrics::MARKET_DATA_BALANCE_UPDATED_FROM_CACHE_TOTAL;
        ...
        let counter_before = MARKET_DATA_BALANCE_UPDATED_FROM_CACHE_TOTAL.load(Ordering::Relaxed);
        ctx.record_momentum_active_pool_reserve_registration_outcome(
            pool,
            GeyserPinReason::MomentumActive,
            false,
            true,
    ...
    #[test]
    fn momentum_pin_already_satisfied_triggers_cache_publish_path() {
        use ironcrab::metrics::MARKET_DATA_BALANCE_UPDATED_FROM_CACHE_TOTAL;
    ...
    fn apply_arb_active_entries_geyser_only_no_cold_rpc() {
        ...
        assert!(
            ...
            "FIX-29 serum backfill requires cold_path_rpc on MarketDataContext"
        );
        assert!(
