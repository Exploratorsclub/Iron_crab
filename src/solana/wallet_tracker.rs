...
use crate::ipc::{
    AlertSeverity, InsiderAlertType, MarketEvent,
    MarketEventKind, RecordHeader, WalletAction, WalletType,
};
    ...
    pub fn process_trade(
        ...
        if is_buy {
            if let Some(event) = self.check_early_buyer(
                mint, trader_wallet, slot, amount_sol, amount_tokens, run_id, component
            ) {
                events.push(event);
            }
        }
        ...
        let (alert_type, severity, description) = match (wallet_type, action) {
            (WalletType::DevWallet, WalletAction::Sell) => {
                (
                    ...
                    format!("Dev wallet {} selling {} lamports of {}", wallet, amount_sol, mint),
                )
            }
            ...
            (WalletType::Whale, WalletAction::Sell) if amount_sol >= self.config.whale_threshold_lamports * 2 => {
                (
                    InsiderAlertType::WhaleDumping,
                    AlertSeverity::Warning,
                    format!("Whale {} dumping {} SOL of {}", wallet, amount_sol / 1_000_000_000, mint),
                )
            }
            _ => return None,
        };
