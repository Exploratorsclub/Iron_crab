...
use super::{Dex, Quote};
use crate::metrics::{LatencyTimer, QUOTE_REQUESTS, QUOTE_SUCCESSES, ROUTER_SINGLE_HOP};
use anyhow::Result;
...
impl Router {
    pub fn new(dexs: Vec<Arc<dyn Dex>>) -> Self {
        Self { dexs }
    }

    pub fn dexs(&self) -> &Vec<Arc<dyn Dex>> {
        &self.dexs
    }
    ...
    pub async fn best_quote_exact_in(
        ...
        output_mint: &str,
        amount_in: u64,
    ) -> Result<Option<RouteQuote>> {
        if self.dexs.is_empty() {
            return Ok(None);
        }
        ...
        let futs = self
            .dexs
            .iter()
            .map(|d| d.quote_exact_in(input_mint, output_mint, amount_in));
        ...
        if best.is_some() {
            QUOTE_SUCCESSES.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
            ROUTER_SINGLE_HOP.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
        }
        Ok(best.map(|(i, q)| RouteQuote {
            dex_index: i,
            quote: q,
        }))
    }
    ...
    pub async fn best_quote_exact_in_hops2(
        &self,
        input_mint: &str,
        output_mint: &str,
        amount_in: u64,
    ) -> Result<Option<(Vec<(usize, Quote)>, u64)>> {
        if self.dexs.is_empty() {
            return Ok(None);
        }
        ...
        for mid in mids {
            ...
            if replace {
                best = Some((vec![(i1, q1), (i2, q2)], total_out));
            }
        }
        Ok(best)
    }
    ...
    pub async fn best_quote_exact_in_hops3(
        &self,
        input_mint: &str,
        output_mint: &str,
        amount_in: u64,
        ...
        for d in &self.dexs {
            for (a, b) in d.list_pairs() {
                if a == input_mint {
                    neighbors_from_input.insert(b.clone());
                }
                if b == input_mint {
                    neighbors_from_input.insert(a.clone());
                }
                if a == output_mint {
                    neighbors_to_output.insert(b.clone());
                }
                if b == output_mint {
                    neighbors_to_output.insert(a.clone());
                }
            }
        }
        ...
        for mid1 in neighbors_from_input.iter() {
            ...
            let (i1, q1) = match hop1_best {
                Some(v) => v,
                None => continue,
            };
            if q1.price_impact_bps > 3000 {
                continue;
            }
            ...
            for d in &self.dexs {
                for (a, b) in d.list_pairs() {
                    if a == *mid1 {
                        mid2_candidates.insert(b.clone());
                    }
                    if b == *mid1 {
                        mid2_candidates.insert(a.clone());
                    }
                }
            }
            for mid2 in mid2_candidates.iter() {
                if mid2 == mid1 || mid2 == input_mint || mid2 == output_mint {
                    continue;
                }
                if !neighbors_to_output.contains(mid2) {
                    continue;
                }
                // hop2
                ...
                let (i2, q2) = match hop2_best {
                    Some(v) => v,
                    None => continue,
                };
                if q2.price_impact_bps > 3000 {
                    continue;
                }
                // hop3
                ...
                let (i3, q3) = match hop3_best {
                    Some(v) => v,
                    None => continue,
                };
                if q3.price_impact_bps > 3000 {
                    continue;
                }
                ...
                if replace {
                    best = Some((
                        vec![(i1, q1.clone()), (i2, q2.clone()), (i3, q3)],
                        total_out,
                    ));
                }
            }
    ...
    pub async fn build_best_hops2_plan_exact_in(
        &self,
        input_mint: &str,
        output_mint: &str,
        amount_in: u64,
        slippage_bps: u32,
    ) -> Result<Option<MultiHopSwapPlan>> {
        let best_path = self
            .best_quote_exact_in_hops2(input_mint, output_mint, amount_in)
            .await?;
        let (hops, final_out) = match best_path {
            Some(v) => v,
            None => return Ok(None),
        };
        let min_out = Self::cumulative_min_out(
            &hops.iter().map(|(_, q)| q.clone()).collect::<Vec<_>>(),
            slippage_bps,
        );
        ...
        if hops.len() == 2 {
            ...
            let hop2_ixs =
                self.dexs[*d2].build_swap_ix(&mid, &q2.output_mint, q1.amount_out, min_out)?;
            ixs.extend(hop1_ixs);
            ixs.extend(hop2_ixs);
        } else {
            // Fallback: single hop path already handled outside; return None.
        }
        Ok(Some(MultiHopSwapPlan {
            ixs,
            hops,
            amount_in,
