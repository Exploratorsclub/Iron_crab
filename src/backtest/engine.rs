use anyhow::Result;
use super::types::{SimEvent,Portfolio,StrategyDecision,BacktestStrategy,StrategyAction,SimContext,SimEventKind,FillInfo};
...
impl<S: BacktestStrategy, M: MarketAdapter> BacktestEngine<S,M> {
    pub fn new(strategy:S, market:M, portfolio:Portfolio, events:Vec<SimEvent>) -> Self {
        Self {
            strategy, market, portfolio, events,
            decisions: vec![], slippage_rejections: vec![], executions: vec![],
            impact_model: None,
    ...
    pub fn set_impact_settings(&mut self, settings: ImpactSettings) {
        self.noise_sampler = Some(NoiseSampler::new(settings.seed, settings.noise_bps_mean, settings.noise_bps_std));
        self.impact_settings = settings;
    }
    pub fn set_slippage_override_bps(&mut self, bps: Option<u32>) { self.slippage_override_bps = bps; }
    pub fn run(&mut self) -> Result<()> {
        // Lifecycle: init once at start
        if let Some(first) = self.events.first() {
            let ictx = SimContext { portfolio: &self.portfolio, time_ms: first.ts_ms };
        ...
        for ev in &self.events {
            ...
            match &ev.kind {
                SimEventKind::NewPool { pool, base_mint, quote_mint, fee_bps } => {
                    self.market.on_new_pool(pool, base_mint, quote_mint, *fee_bps);
                }
                SimEventKind::CfmPriceUpdate { pool, base_reserve, quote_reserve, fee_bps } => {
                    self.market.on_price_update(pool, *base_reserve, *quote_reserve, *fee_bps);
                }
                _ => {}
            }
            // limit immutable borrow of portfolio to this block
            let decision = {
                let ctx = SimContext { portfolio: &self.portfolio, time_ms };
                ...
                match res {
                    ...
                    Err(_) => {
                        ...
                        if self.failure_count >= FAIL_THRESHOLD {
                            ...
                            self.breaker_open_until = Some(Instant::now() + std::time::Duration::from_millis(OPEN_MS));
                        }
            ...
            for act in &decision.actions {
                ...
                StrategyAction::Swap(a) => {
                    ...
                    if a.max_slippage_bps > 0 {
                        let allowed_slip = self.slippage_override_bps.unwrap_or(a.max_slippage_bps);
                        if let Some(q) = self.market.quote(&a.input_mint, &a.output_mint, a.amount_in) {
                            let base_out = if let Some(ref im) = self.impact_model {
                                ...
                                im.expected_out(q.in_reserve, q.out_reserve, a.amount_in, q.fee_bps, q.tick_spacing)
                            } else { q.amount_out };
                            ...
                            if extra_fee > 0 {
                                let adj = (base_after_extra as u128) * (10_000u128 - extra_fee as u128) / 10_000u128;
                            ...
                            if let Some(s) = self.noise_sampler.as_mut() { slippage_bps = slippage_bps.saturating_add(s.sample_bps()); }
                            // Latency penalty: translate emulate_latency_ms into additional bps
                            if let (Some(lat_ms), Some(slot_ms)) = (self.impact_settings.emulate_latency_ms, self.impact_settings.slot_ms) {
                                if slot_ms > 0 {
                    ...
                    if self.impact_settings.extra_fee_bps > 0 {
                        let adj = (out as u128) * (10_000u128 - self.impact_settings.extra_fee_bps as u128) / 10_000u128;
                        out = adj as u64;
                    }
                    if let Some(s) = self.noise_sampler.as_mut() {
                        let nb = s.sample_bps();
                        if nb > 0 { let adj = (out as u128) * (10_000u128 - nb as u128) / 10_000u128; out = adj as u64; }
                    }
                    ...
                    let ctx_fill = SimContext { portfolio: &self.portfolio, time_ms };
        ...
        if let Some(last) = self.events.last() {
            let ectx = SimContext { portfolio: &self.portfolio, time_ms: last.ts_ms };
...
pub mod py_strategy_adapter {
    ...
    enum WorkerMsg {
        Req { json: String, expect_reply: bool, tx: mpsc::Sender<Result<Option<String>, String>> },
        Stop,
    }

    struct PyWorker { tx: mpsc::Sender<WorkerMsg> }

    impl PyWorker {
        fn spawn(cmd: String, args: Vec<String>) -> Arc<Self> {
            let (tx, rx) = mpsc::channel::<WorkerMsg>();
            std::thread::spawn(move || {
                fn start_child(cmd: &str, args: &[String]) -> std::io::Result<(Child, ChildStdin, BufReader<ChildStdout>)> {
                ...
                let mut child_opt: Option<(Child, ChildStdin, BufReader<ChildStdout>)> = start_child(&cmd, &args).ok();
                while let Ok(msg) = rx.recv() {
                    match msg {
                        WorkerMsg::Stop => {
                            if let Some((mut c, _in, _out)) = child_opt.take() { let _ = c.kill(); }
                            break;
                        }
                        WorkerMsg::Req { json, expect_reply, tx } => {
                            // Ensure child exists
                            if child_opt.is_none() {
                                child_opt = start_child(&cmd, &args).ok();
                                crate::metrics::PY_STRAT_RESTARTS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                            }
                            let mut ok = false;
                            if let Some((ref mut ch, ref mut stdin, ref mut stdout)) = child_opt {
                                // write line
                                if writeln!(stdin, "{}", json).is_ok() { let _ = stdin.flush(); } else { let _ = tx.send(Err("write_failed".into())); continue; }
                                if expect_reply {
                                    let mut line = String::new();
                                    match stdout.read_line(&mut line) {
                                        Ok(0) => { let _ = tx.send(Err("eof".into())); }
                                        Ok(_) => { let _ = tx.send(Ok(Some(line))); ok = true; }
                                        Err(e) => { let _ = tx.send(Err(format!("read_err:{e}"))); }
                                    }
                                } else {
                            ...
                            if !ok {
                                ...
                                if let Some((mut c, _, _)) = child_opt.take() { let _ = c.kill(); }
                                child_opt = start_child(&cmd, &args).ok();
                                crate::metrics::PY_STRAT_RESTARTS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                            }
                        }
        ...
        fn request(&self, json: String, expect_reply: bool, timeout: Duration) -> Result<Option<String>, ()> {
            let (tx, rx) = mpsc::channel();
            if self.tx.send(WorkerMsg::Req { json, expect_reply, tx }).is_err() { return Err(()); }
    ...
    impl Drop for PyWorker {
        fn drop(&mut self) { let _ = self.tx.send(WorkerMsg::Stop); }
    }

    impl PyProcStrategy {
        pub fn new(cmd: impl Into<String>, args: Vec<String>) -> Self {
            let worker = PyWorker::spawn(cmd.into(), args.clone());
            Self { cmd: "python".into(), args, timeout_ms: 500, worker, breaker: Mutex::new((0, None)) }
        }
        ...
        pub fn with_python(python_exe: impl Into<String>, script_path: impl Into<String>, extra_args: &[impl AsRef<str>]) -> Self {
            ...
            Self { cmd, args, timeout_ms: 500, worker, breaker: Mutex::new((0, None)) }
        }
        /// Configure per-call timeout in milliseconds (default 500ms)
        pub fn with_timeout_ms(mut self, timeout_ms: u64) -> Self { self.timeout_ms = timeout_ms; self }

    /// Explicitly stop the worker (best-effort). Usually not needed due to Drop.
    pub fn shutdown(&self) { self.worker.stop(); }
    }
    ...
    impl BacktestStrategy for PyProcStrategy {
        ...
        fn on_tick(&self, ctx: &SimContext) -> StrategyDecision {
            ...
            if let Some(until) = br.1 { if Instant::now() < until { return StrategyDecision { actions: vec![] }; } else { br.1 = None; } }
            drop(br);
            let req = serde_json::json!({"kind":"tick","time_ms": ctx.time_ms});
            let res = self.worker.request(req.to_string(), true, Duration::from_millis(self.timeout_ms));
        match res {
                Ok(Some(s)) => {
                    if let Ok(dec) = serde_json::from_str::<StrategyDecision>(&s) { self.breaker.lock().unwrap().0 = 0; return dec; }
                    let mut br = self.breaker.lock().unwrap();
                    br.0 = br.0.saturating_add(1);
            crate::metrics::PY_STRAT_FAILS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    if br.0 >= 5 { br.0 = 0; br.1 = Some(Instant::now() + Duration::from_millis(5_000)); crate::metrics::PY_STRAT_CIRCUIT_OPENS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed); }
                    StrategyDecision { actions: vec![] }
                }
                _ => {
                    let mut br = self.breaker.lock().unwrap();
                    br.0 = br.0.saturating_add(1);
            crate::metrics::PY_STRAT_TIMEOUTS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    if br.0 >= 5 { br.0 = 0; br.1 = Some(Instant::now() + Duration::from_millis(5_000)); crate::metrics::PY_STRAT_CIRCUIT_OPENS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed); }
                    StrategyDecision { actions: vec![] }
                }
            }
        ...
        fn on_event(&self, _ctx: &SimContext, event: &SimEvent) -> StrategyDecision {
            ...
            let res = self.worker.request(ev_json, true, Duration::from_millis(self.timeout_ms));
        match res {
                Ok(Some(s)) => {
                    if let Ok(dec) = serde_json::from_str::<StrategyDecision>(&s) { self.breaker.lock().unwrap().0 = 0; return dec; }
                    let mut br = self.breaker.lock().unwrap();
                    br.0 = br.0.saturating_add(1);
            crate::metrics::PY_STRAT_FAILS_TOTAL.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                    if br.0 >= 5 { br.0 = 0; br.1 = Some(Instant::now() + Duration::from_millis(5_000)); }
                    StrategyDecision { actions: vec![] }
                }
                _ => {
                    let mut br = self.breaker.lock().unwrap();
        ...
        fn on_fill(&self, ctx: &SimContext, fill: &FillInfo) {
            ...
            let _ = self.worker.request(req.to_string(), false, Duration::from_millis(self.timeout_ms));
        }
    ...
    impl Drop for PyProcStrategy {
        fn drop(&mut self) { self.worker.stop(); }
    }
}

pub struct NoopStrategy;
impl BacktestStrategy for NoopStrategy {
    fn on_event(&self, _ctx:&SimContext, _ev:&SimEvent) -> StrategyDecision { StrategyDecision { actions: vec![] } }
}
