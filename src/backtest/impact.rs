...
impl Default for ImpactSettings {
    fn default() -> Self {
    Self { seed: None, noise_bps_mean: 0.0, noise_bps_std: 0.0, emulate_latency_ms: None, extra_fee_bps: 0, slot_ms: Some(400) }
    }
}
...
impl NoiseSampler {
    pub fn new(seed: Option<u64>, mean: f32, std: f32) -> Self {
        let rng = match seed { Some(s) => SmallRng::seed_from_u64(s), None => SmallRng::from_entropy() };
        Self { rng, mean, std }
    }
    /// Sample a shortfall in bps (>= 0). Uses a truncated normal at 0.
    pub fn sample_bps(&mut self) -> u32 {
        if self.std <= 0.0 { return self.mean.max(0.0) as u32; }
        // Box-Muller transform for normal sampling
    let mut u1: f32 = self.rng.gen();
        if u1 < 1.0e-6 { u1 = 1.0e-6; }
        if u1 > 1.0 - 1.0e-6 { u1 = 1.0 - 1.0e-6; }
