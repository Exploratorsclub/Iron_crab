...
pub struct CfmAdapter {
    pub pools: Vec<CfmPool>,
}
impl CfmAdapter {
    pub fn new() -> Self {
        Self { pools: vec![] }
    ...
    pub fn ingest_raydium(&mut self, snaps: &[PoolSnapshot]) {
        for s in snaps {
            self.upsert_pool(CfmPool {
                pool: s.address.to_string(),
                base_mint: s.base_mint.to_string(),
                quote_mint: s.quote_mint.to_string(),
                base_reserve: s.reserve_base,
                quote_reserve: s.reserve_quote,
                fee_bps: s.fee_bps,
                tick_spacing: None,
            });
        }
...
impl MarketAdapter for CfmAdapter {
    fn apply_swap(&mut self, action: &ActionSwap) -> Result<(u64, u64)> {
        let pool = self
            ...
            .iter_mut()
            .find(|p| {
                (p.base_mint == action.input_mint && p.quote_mint == action.output_mint)
                    || (p.base_mint == action.output_mint && p.quote_mint == action.input_mint)
            })
            .ok_or_else(|| anyhow!("pool not found"))?;
        let forward = pool.base_mint == action.input_mint;
        let (rin, rout) = if forward {
            (&mut pool.base_reserve, &mut pool.quote_reserve)
        } else {
            (&mut pool.quote_reserve, &mut pool.base_reserve)
        };
        if *rin == 0 || *rout == 0 {
    ...
    fn quote(&self, input_mint: &str, output_mint: &str, amount_in: u64) -> Option<Quote> {
        let p = self.pools.iter().find(|p| {
            (p.base_mint == input_mint && p.quote_mint == output_mint)
                || (p.base_mint == output_mint && p.quote_mint == input_mint)
        })?;
        ...
        Some(Quote {
            ...
            price_impact_bps: impact as u32,
            route: vec![p.pool.clone()],
            fee_bps: p.fee_bps,
            in_reserve: rin,
            out_reserve: rout,
            input_mint: (if forward {
                p.base_mint.clone()
            } else {
                p.quote_mint.clone()
            }),
            output_mint: (if forward {
                p.quote_mint.clone()
            } else {
                p.base_mint.clone()
            }),
    ...
    fn on_new_pool(&mut self, pool: &str, base_mint: &str, quote_mint: &str, fee_bps: u32) {
        ...
        if self.pools.iter().any(|p| p.pool == pool) {
            return;
        }
        self.pools.push(CfmPool {
            pool: pool.to_string(),
...
impl ImpactModel for ClmmModel {
    fn expected_out(
        ...
        let base = CpmMModel.expected_out(in_reserve, out_reserve, amount_in, fee_bps, tick_spacing)
            as u128;
        if in_reserve == 0 {
            return 0;
        }
        ...
        let ticks = if spacing_bps == 0 {
            0
        } else {
            move_bps / spacing_bps
        };
