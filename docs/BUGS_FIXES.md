...
### PR155-PUBLISH-RUNTIME-ISOLATION: eigener Tokio-Runtime-Thread + Dispatcher ohne Mutex-Recv + NATS-Reconnect + Liveness-Watchdog
**Datum**: 2026-05-26  
**Problem** (Prod nach Deploy `090be31` / PR #154): Kurzer Burst (~25 s, ~74k Jobs), danach **60+ s** flache `nats_messages_published_total`, `publish_worker_stalls_total=0`, Queue z. B. 2221, `nats sub` **0** Messages — Geyser/Ingest lief weiter. Ursachen: (1) Publish-Worker liefen auf dem **Haupt-Tokio-Runtime**; viele parallele hängende `publish().await`-Tasks **sättigten** den Default-Thread-Pool → **Timer-Starvation** (weder 100 ms NATS-Timeout noch 2 s Worker-`timeout` feuerten). (2) **`Arc<Mutex<Receiver>>`** um den gemeinsamen `mpsc::Receiver` — nur ein Worker konnte `recv().await`, Flaschenhals + wachsende Queue. Dedizierte NATS-TCP allein reicht nicht, solange Publish im selben Runtime-Kontext wie Geyser-Ingest konkurriert.  
**Fix**: (1) Publish-Pipeline ausschließlich in **`std::thread` `md-publish`** mit **`tokio::runtime::Builder::new_multi_thread().worker_threads(MARKET_DATA_PUBLISH_WORKER_COUNT).thread_name("md-publish")`** — Haupt-Runtime: Geyser/Account/TX, **kein** `account_path_publish_worker` mehr dort. (2) **Single-Dispatcher**-Task: `recv` auf dem **einzigen** Inbound-`mpsc`, Round-Robin-Fan-out auf **N** Worker-Channels — **kein** `Mutex` über `recv().await`. (3) Nach **Job-Timeout** / Join-Fehler: `abort` beibehalten, **`NatsClient::reconnect()`** (disconnect + `connect` gleiche URL/Name) + Metrik `market_data_account_publish_worker_reconnects_total`. (4) **Liveness-Watchdog** alle 10 s: wenn `queue_depth > 0` und `now - last_success_unix_ms > 30 s` → **alle** Publish-NATS-Clients reconnect + Warn-Log + `market_data_account_publish_worker_liveness_reconnects_total`. (5) PR #154 beibehalten: Enqueue-`ctx=None`-Side-Effects, JetStream-Drop ab Depth ≥8192, `spawn`+`timeout`+`abort` pro Job — nur im isolierten Runtime. **Invarianten**: kein neuer RPC (I-7), keine NATS-Topic-Änderung (I-23).  
**Dateien**: `src/bin/market_data.rs`, `src/metrics.rs`, `src/nats/client.rs`, `docs/BUGS_FIXES.md`

### PR154-PUBLISH-WORKER-SPAWN-ABORT: Tokio-Timeout wirksam + kein `bonding_curve_publish_times` im Worker + Multi-Worker + JetStream-Drop
**Datum**: 2026-05-26  
**Problem** (Prod nach PR #153): `tokio::time::timeout(2s, process_job()).await` auf **demselben** Task wie der Job — blockiert der Job den Worker-Thread ohne `.await` (z. B. `parking_lot::Mutex` mit Account-Handler, synchrones CPU, hängendes NATS-I/O), wird der **Runtime-Timer nicht gepollt** → kein Timeout, `publish_worker_stalls_total=0`, `last_success` veraltet, Publish-Queue eingefroren (`nats_messages_published_total` flat).  
**Fix**: (1) Pro Job: `tokio::spawn` + `tokio::time::timeout` auf `JoinHandle`, bei Ablauf `abort_handle.abort()` und `stalls_total++`. (2) `CoreMarketEvent`-Publish im Worker nur mit `ctx=None` bei `publish_market_event_core_and_momentum_ex`; Bonding-/Trade-Metriken und `bonding_curve_publish_times` **optimistisch** beim Enqueue (`apply_market_event_publish_success_side_effects`). (3) **`MARKET_DATA_PUBLISH_WORKER_COUNT`** (Default **4**) parallele Worker, je **eigene** NATS-TCP-Verbindung `market-data-publish-{i}`, gemeinsamer `mpsc::Receiver` unter `Arc<tokio::sync::Mutex<_>>`. (4) Backpressure: bei `market_data_account_publish_queue_depth` ≥ **8192** neue **JetStream**-Enqueues verwerfen (`market_data_account_publish_queue_dropped_total++`); **kein** Drop von `CoreMarketEvent`. (5) Metriken: `market_data_account_publish_workers_active`, `market_data_account_publish_queue_dropped_total`. (6) Tests: `TestBlockMs` (sync `thread::sleep` im async Job), Stall-Counter-Assertion, JetStream-Drop, Enqueue-only mit `publish_tx`. **Invarianten**: kein neuer RPC (I-7), keine NATS-Topic-/Payload-Änderung (I-23), `publish_timeout` global unverändert.  
**Dateien**: `src/bin/market_data.rs`, `src/metrics.rs`, `docs/BUGS_FIXES.md`

### PR153-NATS-PUBLISH-PIPELINE-UNSTALL: dedizierter Publish-Client + Worker-Job-Timeout + vollständiger TX/Main-Enqueue
**Datum**: 2026-05-26  
**Problem** (Prod nach PR #152 / Deploy `c4cfdbc`): `nats_messages_published_total` stieg kurz nach Restart, frierte dann ein; `nats sub ironcrab.v1.market_events` lieferte **0** Messages; `market_data_account_publish_queue_depth` blieb konstant (z. B. 11). Watchdog war stabil — Stall **vor** Timeout/Drop-Logs: gemischtes Publish auf **derselben** `async_nats::Client`-Verbindung (`clone_for_spawned_publish` teilt den Client): synchrones `publish_market_event_core_and_momentum_ex().await` / `jetstream_publish` im **TX-Task** und im **Main-`select!`** blockierte gemeinsam mit dem einen Publish-Worker.  
**Fix**: (1) **Alle** verbleibenden Hot-Path-Publishes im TX-Task und Pool-Discovery im Main-Loop gehen über `account_publish_tx` (`account_path_enqueue_*`) — kein direktes Await auf Core-Publish/JetStream dort, wenn die Queue existiert. `TopicPublish`-Job für Priority-Fee-Samples; Wallet-Events als `CoreMarketEvent`. (2) Publish-Worker: **eigene** TCP-Verbindung `NatsConfig::new(nats_url, "market-data-publish")` vor `tokio::spawn` (kein Shared-Client mit Geyser-Tasks); bei Connect-Fehler bleibt die Queue aus (`error!`). (3) Pro-Job-`tokio::time::timeout(2 s)` um `process_account_path_nats_job`; bei Timeout: Job verwerfen, `market_data_account_publish_worker_stalls_total++`, nächster Job. (4) Metriken: `market_data_account_publish_worker_job_duration_us`, `market_data_account_publish_worker_last_success_unix_ms`. (5) Geyser→Publish-Latenz-Histogramm: bei Enqueue aufgenommen (`skip_geyser_to_publish_histogram` im Worker). **Invarianten**: kein neuer RPC (I-7), keine NATS-Topic-/Payload-Änderung (I-23), `publish_timeout` global unverändert.  
**Dateien**: `src/bin/market_data.rs`, `src/metrics.rs`, `docs/BUGS_FIXES.md`
