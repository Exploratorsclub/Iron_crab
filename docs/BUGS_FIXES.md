...
### FIX-MOM-INGEST-FAIRNESS-2: `select!`-Bias, kürzere JetStream-Leerpolls, adaptiver Core-NATS-Drain + Backlog-Metriken
**Datum**: 2026-05-15  
**Problem**: Trotz Batch-Drain blieb der effektive Core-NATS-Durchsatz deutlich unter der Publish-Rate: JetStream-Arme mit langem `expires`, der ständig startende geplante `ExecutionResult`-Fetch und `select!`-Fairness begünstigten andere Zweige; festes Drain-Cap (48) war oft gesättigt, ohne dass die Hot-Path-Zeit pro Event hoch war.  
**Fix**: `tokio::select! { biased; }` mit Watchdog/Shutdown zuerst, danach Core-MarketEvents; geplanter `ExecutionResult`-Drain nur noch auf `interval(EXECUTION_RESULT_SCHEDULED_POLL_INTERVAL)` statt bei jeder Schleifenrunde; kürzere `expires` für PoolCache-/Config-/Wallet-JetStream-Leerpfade; höheres Boden-Drain-Cap (96) plus adaptives Cap bis 320 anhand Slot-Backlog (`latest_seen - last_processed`); Prometheus: Slot-Backlog, Drain-Batch-/Cap-Hit-Zähler, `event_to_ingest`-Sum-Cap-Sample-Counter, per-Kind received/processed (ohne dynamische Labels).  
