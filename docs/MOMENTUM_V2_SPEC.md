...
**Quote-first price exits (STOP_LOSS / TAKE_PROFIT / TRAILING_STOP):** these fire only when a usable **executable reserve quote** breaches the configured threshold (TRAILING_STOP trigger requires `marks_position_pool`, I-13). **Current-price-only** triggers are disabled. **TRAILING_STOP activation** follows peak PnL implied by the **trade-session high** on the position pool (entry fill + post-entry trade-derived `tokens_per_sol`, slot-gated), not PoolCache reserve marks. **Scale-in baseline reset** (in `add_investment`): overwrites `highest_price` / `trade_mark_tps` / `current_price` to the scale-in fill tps, sets `trailing_active = false`, `entry_confirmed_slot` and `last_price_slot` to the scale-in confirm slot, clears `recent_trades`; `entry_price` remains SOL-weighted across tranches.
