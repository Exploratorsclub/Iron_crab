...
**Quote-first price exits (STOP_LOSS / TAKE_PROFIT / TRAILING_STOP):** these fire only when a usable **executable reserve quote** breaches the configured threshold. **Current-price-only** triggers for those exits are disabled; the trade-ratio / reserve mark may diverge for logging but is not the trigger source. **TRAILING_STOP** uses two layers: **activation** follows peak PnL implied by the **trade-session high** on the position pool (entry fill + post-entry **BUY** trade-derived `tokens_per_sol` on that pool, slot-gated), not PoolCache reserve marks and not the executable quote alone. **SELL** trades may refresh the **mark** (`current_price`) but must **not** improve session high. **Dust BUYs** below `min_session_high_trade_sol` (config, default 0.05 SOL notional) still update the mark but do **not** move session high — keeps trailing/chart alignment. **Activation** may also latch when the **guarded executable** PnL alone reaches `trailing_activation_pct` (so real sellability can arm trailing even if session-high moves are filtered). **Trigger** drawdown is quote-first (guarded executable vs session high). The executable must **mark the position pool** for the trailing drawdown comparison (I-13). **Reserve hints** advance `last_reserve_hint_slot` / timestamp only — they must not advance `last_trade_slot`, so a trade in the **same slot** as a reserve mark is still merged into the position. After BUY confirm, the bot **backfills** pool trades already present on the token tracker with slots in `(entry_confirmed_slot, chain_head]` (no RPC) so post-entry prints are not lost to ordering races. **TIME_EXIT / MOMENTUM_EXIT / BONDING_CURVE_EXIT** reason strings prefer **executable** PnL when a structural quote exists, with mark PnL in parentheses.
